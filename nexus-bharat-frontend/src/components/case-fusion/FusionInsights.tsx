...
interface FusionInsightsProps {
  insights: FusionInsight[];
}

export const FusionInsights: React.FC<FusionInsightsProps> = ({ insights }) => {
      ...
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-xs font-bold uppercase tracking-wider text-slate-200">
            Synthesized Intelligence Discoveries
          </h3>
        </div>
        <span className="text-[10px] font-mono px-2 py-0.5 rounded-full bg-cyan-950/60 border border-cyan-800/50 text-cyan-300 font-semibold">
          {insights.length} Actionable Theses
        </span>
      </div>
        ...
        {insights.map((f) => {
          ...
          return (
            ...
            >
              ...
              {isExpanded && (
                <div className="mt-2.5 pt-2.5 border-t border-slate-800/80 text-xs text-slate-300 leading-relaxed animate-in fade-in duration-150">
                  <p>{f.description}</p>
                </div>
              )}
            </div>
